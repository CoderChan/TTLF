...
/**
 *设定当前地图的显示范围
 *参数 region 要设定的地图范围，用经纬度的方式表示
 *参数 animated 是否采用动画效果
 */
- (void)setRegion:(BMKCoordinateRegion)region animated:(BOOL)animated;

/**
 *设定地图中心点坐标
 *参数 coordinate 要设定的地图中心点坐标，用经纬度表示
 *参数 animated 是否采用动画效果
 */
...
/**
 *将View矩形区域转换成经纬度矩形区域
 *参数 rect 待转换的View矩形区域
 *参数 view rect坐标所在的view
 *@return 转换后的经纬度矩形区域
 */
 ...
 *批量将BMKMapPoint转换为opengles可以直接使用的坐标
 参数 mapPoints BMKMapPoint坐标数据指针
 参数 count 个数,count不能大于数组长度
...
/**
 * 设置地图中心点在地图中的屏幕坐标位置
 * 参数 ptInScreen 要设定的地图中心点位置，为屏幕坐标，设置的中心点不能超过屏幕范围，否则无效
 */
 ...
 *	设置地图状态
 *	参数	[in]	mapStatus	地图状态信息
 *	参数	[in]	bAnimation	是否需要动画效果，true:需要做动画
 */
 ...
 *参数 annotations 要添加的标注数组
 */
- (void)addAnnotations:(NSArray *)annotations;

/**
 *移除标注
 *参数 annotation 要移除的标注
 */
 ...
 *参数 overlay 带添加的Overlay
 *参数 sibling 用于指定相对位置的Overlay
 */
...
/**
 *添加热力图
 *	参数	[BMKHeatMap*]	heatMap	热力图绘制和显示数据
 */
 ...
 *参数 mapView 地图View
 *参数 views 取消选中的annotation views
 */
 ...
 *参数 mapView 地图View
 *参数 overlayViews 新添加的overlay views
 */
 ...
 *参数 mapview 地图View
 *参数 coordinate 触摸点的经纬度
 *参数 force 触摸该点的力度(参考UITouch的force属性)
 *参数 maximumPossibleForce 当前输入机制下的最大可能力度(参考UITouch的maximumPossibleForce属性)
 */
