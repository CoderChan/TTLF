...
- (void)setItem:(KSPhotoItem *)item determinate:(BOOL)determinate {
    ...
    if (item) {
        ...
        [_imageView sd_setImageWithURL:item.imageUrl placeholderImage:[UIImage imageWithColor:HWRandomColor] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            __strong typeof(wself) sself = wself;
            if (!error) {
