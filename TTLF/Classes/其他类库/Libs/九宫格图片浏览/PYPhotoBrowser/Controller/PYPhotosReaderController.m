...
- (void)deviceOrientationDidChange
{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        self.pageLabel.centerX = width * 0.5;
        self.pageLabel.y = height - 54;
        // 刷新数据
        [self.collectionView reloadData];
        NSInteger photosCount = self.selectedPhotoView.photos.count;
        self.collectionView.contentSize = CGSizeMake(self.collectionView.width * photosCount, self.collectionView.height);
        self.collectionView.contentOffset = CGPointMake(self.selectedPhotoView.tag * self.collectionView.width, 0);
    } completion:^(BOOL finished) {
        self.rotating = NO;
...
{
    if (scrollView.contentOffset.x != self.selectedPhotoView.tag * self.collectionView.width && PYIOS8 && !self.dragging) { // 修复在iOS8系统下，scrollView.contentOffset被系统又初始化的BUG
        scrollView.contentOffset = CGPointMake(self.selectedPhotoView.tag * self.collectionView.width, 0);
    }
