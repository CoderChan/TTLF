...
- (void)setPhotos:(NSArray<PYPhoto *> *)photos
{
    ...
    CGFloat width = size.width + self.x > PYScreenW ? PYScreenW - self.x : size.width;
...
- (void)layoutSubviews
{
    ...
    if (self.images.count < self.imagesMaxCountWhenWillCompose && self.photosState == PYPhotosViewStateWillCompose) {
        ...
        self.addImageButton.y = (self.images.count / maxCol) * (self.photoHeight + self.photoMargin);
        self.addImageButton.x = (self.images.count % maxCol) * (self.photoWidth + self.photoMargin);
        self.addImageButton.width = self.photoWidth;
        self.addImageButton.height = self.photoHeight;
        if (self.images.count == 0) { // 数组为空
            self.size = self.addImageButton.size;
        }
    }
}
