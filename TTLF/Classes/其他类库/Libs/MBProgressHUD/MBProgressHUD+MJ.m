...
{
    [self show:warn icon:@"hud_warn" view:view];
}
+ (void)showStore:(NSString *)store Type:(BOOL)isStore toView:(UIView *)view
{
    if (isStore) {
        [self show:store icon:@"hud_store" view:view];
    }else{
        [self show:store icon:@"hud_dis_store" view:view];
    }
}
